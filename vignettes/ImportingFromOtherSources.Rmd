...
## Direct Seurat/Signac input (recommended) {#seurat-direct}
...
## Importing from SnapATAC2 (recommended) {#snapatac2}

If you have a SnapATAC2 `AnnData` or `AnnDataSet` object (or an `.h5ad` file) with cell type labels in `obs`, use `import_snap_atac()` to convert to MOCHA inputs. This requires optional R package **reticulate** and Python package **anndata** (or **snapatac2** for `AnnDataSet`).

```{r}
library(MOCHA)

# adata: SnapATAC2 AnnData after clustering and labeling (obs must include Sample + cell type)
# Or: inputs <- MOCHA::import_snap_atac("my_study.h5ad", cellPopLabel = "leiden", sampleColumn = "Sample")
...
You must provide `genome` and `blackList` to `callOpenTiles()`; `import_snap_atac()` does not infer them. Ensure chromosome names in your SnapATAC2 object match your BSgenome (use `chromPrefix = "add"` if needed).

## Importing from legacy SnapATAC (v1) {#snapatac}

...
Use this checklist when `bioc/submission` passes `R CMD check` and
`BiocCheck` on devel.

## Prerequisites

Maintainer subscribed to [bioc-devel mailing
...
1.  Fork or use
    [Bioconductor/Contributions](https://github.com/Bioconductor/Contributions).
...
4.  Confirm the package builds on the [Single Package
    Builder](https://bioconductor.org/checkResults/) after the issue is
    opened.

## After acceptance

- Add remote:
  `git remote add upstream git@git.bioconductor.org:packages/MOCHA.git`
